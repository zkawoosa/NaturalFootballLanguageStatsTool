// AUTO-GENERATED FILE. Do not edit manually.
// Generated at: 2026-03-10T01:05:00.808Z
// Source: Ball Don't Lie NFL API (https://api.balldontlie.io/nfl/v1)
// Requests: 5
// Players collected: 500

export const GENERATED_TEAM_ALIAS_MAP: Record<string, string[]> = {
  "49ers": ["SF"],
  ari: ["ARI"],
  arizona: ["ARI"],
  atl: ["ATL"],
  atlanta: ["ATL"],
  bal: ["BAL"],
  baltimore: ["BAL"],
  bears: ["CHI"],
  bengals: ["CIN"],
  bills: ["BUF"],
  broncos: ["DEN"],
  browns: ["CLE"],
  buccaneers: ["TB"],
  bucs: ["TB"],
  buf: ["BUF"],
  buffalo: ["BUF"],
  car: ["CAR"],
  cardinals: ["ARI"],
  carolina: ["CAR"],
  chargers: ["LAC"],
  chi: ["CHI"],
  chicago: ["CHI"],
  chiefs: ["KC"],
  cin: ["CIN"],
  cincinnati: ["CIN"],
  cle: ["CLE"],
  cleveland: ["CLE"],
  colts: ["IND"],
  commanders: ["WSH"],
  commandos: ["WSH"],
  cowboys: ["DAL"],
  dal: ["DAL"],
  dallas: ["DAL"],
  den: ["DEN"],
  denver: ["DEN"],
  det: ["DET"],
  detroit: ["DET"],
  dolphins: ["MIA"],
  eagles: ["PHI"],
  england: ["NE"],
  falcons: ["ATL"],
  gb: ["GB"],
  giants: ["NYG"],
  gnb: ["GB"],
  green: ["GB"],
  "green bay": ["GB"],
  hou: ["HOU"],
  houston: ["HOU"],
  ind: ["IND"],
  indianapolis: ["IND"],
  jacksonville: ["JAX"],
  jaguars: ["JAX"],
  jax: ["JAX"],
  jets: ["NYJ"],
  kansas: ["KC"],
  "kansas city": ["KC"],
  kc: ["KC"],
  lac: ["LAC"],
  lar: ["LAR"],
  "las vegas": ["LV"],
  lions: ["DET"],
  "los angeles chargers": ["LAC"],
  "los angeles rams": ["LAR"],
  lv: ["LV"],
  mia: ["MIA"],
  miami: ["MIA"],
  min: ["MIN"],
  minnesota: ["MIN"],
  ne: ["NE"],
  "new york giants": ["NYG"],
  "new york jets": ["NYJ"],
  niners: ["SF"],
  no: ["NO"],
  nola: ["NO"],
  nyg: ["NYG"],
  nyj: ["NYJ"],
  orleans: ["NO"],
  packers: ["GB"],
  panthers: ["CAR"],
  patriots: ["NE"],
  phi: ["PHI"],
  philadelphia: ["PHI"],
  pit: ["PIT"],
  pittsburgh: ["PIT"],
  raiders: ["LV"],
  rams: ["LAR"],
  ravens: ["BAL"],
  saints: ["NO"],
  "san francisco": ["SF"],
  sea: ["SEA"],
  seahawks: ["SEA"],
  seattle: ["SEA"],
  sf: ["SF"],
  sfo: ["SF"],
  steelers: ["PIT"],
  tampa: ["TB"],
  tb: ["TB"],
  ten: ["TEN"],
  tennessee: ["TEN"],
  texans: ["HOU"],
  titans: ["TEN"],
  united: ["DAL", "LAR"],
  vegas: ["LV"],
  vikings: ["MIN"],
  was: ["WSH"],
  washington: ["WSH"],
  wsh: ["WSH"],
};

export const GENERATED_PLAYER_ALIAS_MAP: Record<string, string[]> = {
  "a j brown": ["a j brown"],
  "a t perry": ["a t perry"],
  "aaron jones sr": ["aaron jones sr"],
  "aaron rodgers": ["aaron rodgers"],
  abanikanda: ["israel abanikanda"],
  abdullah: ["ameer abdullah"],
  achane: ["de'von achane"],
  "adam prentice": ["adam prentice"],
  "adam thielen": ["adam thielen"],
  "adam trautman": ["adam trautman"],
  adams: ["davante adams"],
  addison: ["jordan addison"],
  agholor: ["nelson agholor"],
  agnew: ["jamal agnew"],
  ahmed: ["salvon ahmed"],
  "aidan o connell": ["aidan o'connell"],
  "aidan o'connell": ["aidan o'connell"],
  aiyuk: ["brandon aiyuk"],
  "aj dillon": ["aj dillon"],
  "aj mccarron": ["aj mccarron"],
  akers: ["cam akers"],
  akins: ["jordan akins"],
  "alec ingold": ["alec ingold"],
  "alec pierce": ["alec pierce"],
  "alex erickson": ["alex erickson"],
  "alexander mattison": ["alexander mattison"],
  "alie cox": ["mo alie cox"],
  "allen lazard": ["allen lazard"],
  "allen robinson": ["allen robinson"],
  allgeier: ["tyler allgeier"],
  "alvin kamara": ["alvin kamara"],
  "amani hooker": ["amani hooker"],
  "amari cooper": ["amari cooper"],
  "ameer abdullah": ["ameer abdullah"],
  "amon ra st brown": ["amon ra st brown"],
  "andrei iosivas": ["andrei iosivas"],
  "andrew beck": ["andrew beck"],
  andrews: ["mark andrews"],
  "andy dalton": ["andy dalton"],
  "anthony richardson sr": ["anthony richardson sr"],
  "antonio gibson": ["antonio gibson"],
  "ashtyn davis": ["ashtyn davis"],
  atwell: ["tutu atwell"],
  "austin ekeler": ["austin ekeler"],
  "austin hooper": ["austin hooper"],
  "austin iii": ["calvin austin iii"],
  bagent: ["tyson bagent"],
  "bailey zappe": ["bailey zappe"],
  "baker mayfield": ["baker mayfield"],
  "bam knight": ["bam knight"],
  bateman: ["rashod bateman"],
  bates: ["john bates"],
  bawden: ["nick bawden"],
  beathard: ["c j beathard"],
  beck: ["andrew beck"],
  "beckham jr": ["odell beckham jr"],
  bellinger: ["daniel bellinger"],
  "ben skowronek": ["ben skowronek"],
  berrios: ["braxton berrios"],
  bigsby: ["tank bigsby"],
  "bijan robinson": ["bijan robinson"],
  blackshear: ["raheem blackshear"],
  "blaine gabbert": ["blaine gabbert"],
  "blake gillikin": ["blake gillikin"],
  blasingame: ["khari blasingame"],
  "bo melton": ["bo melton"],
  bobo: ["jake bobo"],
  bolden: ["brandon bolden"],
  boone: ["mike boone"],
  "boston scott": ["boston scott"],
  bourne: ["kendrick bourne"],
  "bowden jr": ["lynn bowden jr"],
  boyd: ["tyler boyd"],
  boyle: ["tim boyle"],
  "braden mann": ["braden mann"],
  "brandin cooks": ["brandin cooks"],
  "brandon aiyuk": ["brandon aiyuk"],
  "brandon bolden": ["brandon bolden"],
  "brandon johnson": ["brandon johnson"],
  "brandon powell": ["brandon powell"],
  "braxton berrios": ["braxton berrios"],
  "breece hall": ["breece hall"],
  breida: ["matt breida"],
  "brett rypien": ["brett rypien"],
  "brevin jordan": ["brevin jordan"],
  "brian hoyer": ["brian hoyer"],
  "brian robinson jr": ["brian robinson jr"],
  bridgewater: ["teddy bridgewater"],
  brightwell: ["gary brightwell"],
  brissett: ["jacoby brissett"],
  "brock purdy": ["brock purdy"],
  "brock wright": ["brock wright"],
  browning: ["jake browning"],
  brownlee: ["jason brownlee"],
  bryant: ["harrison bryant"],
  "bryce young": ["bryce young"],
  "brycen hopkins": ["brycen hopkins"],
  burks: ["treylon burks"],
  burrow: ["joe burrow"],
  burton: ["michael burton"],
  "byron pringle": ["byron pringle"],
  "c j beathard": ["c j beathard"],
  "c j ham": ["c j ham"],
  "c j stroud": ["c j stroud"],
  "c j uzomah": ["c j uzomah"],
  cabinda: ["jason cabinda"],
  "cade otton": ["cade otton"],
  "calvin austin iii": ["calvin austin iii"],
  "calvin ridley": ["calvin ridley"],
  "cam akers": ["cam akers"],
  "cameron johnston": ["cameron johnston"],
  campbell: ["parris campbell"],
  carr: ["derek carr"],
  "carson wentz": ["carson wentz"],
  "case keenum": ["case keenum"],
  "cedric tillman": ["cedric tillman"],
  "cedrick wilson jr": ["cedrick wilson jr"],
  "ceedee lamb": ["ceedee lamb"],
  chandler: ["ty chandler"],
  charbonnet: ["zach charbonnet"],
  chark: ["dj chark"],
  "charlie jones": ["charlie jones"],
  "charlie kolar": ["charlie kolar"],
  chase: ["ja'marr chase"],
  "chase brown": ["chase brown"],
  "chase claypool": ["chase claypool"],
  "chase edmonds": ["chase edmonds"],
  "chig okonkwo": ["chig okonkwo"],
  chosen: ["robbie chosen"],
  "chris brooks": ["chris brooks"],
  "chris conley": ["chris conley"],
  "chris evans": ["chris evans"],
  "chris godwin jr": ["chris godwin jr"],
  "chris moore": ["chris moore"],
  "chris olave": ["chris olave"],
  "chris rodriguez jr": ["chris rodriguez jr"],
  "christian kirk": ["christian kirk"],
  "christian mccaffrey": ["christian mccaffrey"],
  "christian watson": ["christian watson"],
  "chuba hubbard": ["chuba hubbard"],
  chubb: ["nick chubb"],
  claypool: ["chase claypool"],
  "clayton tune": ["clayton tune"],
  clifford: ["sean clifford"],
  "clyde edwards helaire": ["clyde edwards helaire"],
  "colby parkinson": ["colby parkinson"],
  "cole kmet": ["cole kmet"],
  "cole turner": ["cole turner"],
  collins: ["nico collins"],
  conklin: ["tyler conklin"],
  conley: ["chris conley"],
  conner: ["james conner"],
  "connor heyward": ["connor heyward"],
  cook: ["dalvin cook"],
  "cook iii": ["james cook iii"],
  cooke: ["logan cooke"],
  cooks: ["brandin cooks"],
  cooper: ["amari cooper"],
  "cooper kupp": ["cooper kupp"],
  "cooper rush": ["cooper rush"],
  corbin: ["jashaun corbin"],
  "cordarrelle patterson": ["cordarrelle patterson"],
  "courtland sutton": ["courtland sutton"],
  cousins: ["kirk cousins"],
  cracraft: ["river cracraft"],
  "craig reynolds": ["craig reynolds"],
  crowder: ["jamison crowder"],
  cunningham: ["malik cunningham"],
  "curtis samuel": ["curtis samuel"],
  "d andre swift": ["d'andre swift"],
  "d ernest johnson": ["d'ernest johnson"],
  "d j montgomery": ["d j montgomery"],
  "d onta foreman": ["d'onta foreman"],
  "d'andre swift": ["d'andre swift"],
  "d'ernest johnson": ["d'ernest johnson"],
  "d'onta foreman": ["d'onta foreman"],
  "dak prescott": ["dak prescott"],
  dallas: ["deejay dallas"],
  "dallas goedert": ["dallas goedert"],
  dalton: ["andy dalton"],
  "dalton kincaid": ["dalton kincaid"],
  "dalton schultz": ["dalton schultz"],
  "dalvin cook": ["dalvin cook"],
  "dameon pierce": ["dameon pierce"],
  "damien harris": ["damien harris"],
  "damien williams": ["damien williams"],
  "daniel bellinger": ["daniel bellinger"],
  "daniel jones": ["daniel jones"],
  "dare ogunbowale": ["dare ogunbowale"],
  "darius slayton": ["darius slayton"],
  "darnell mooney": ["darnell mooney"],
  "darnell washington": ["darnell washington"],
  darnold: ["sam darnold"],
  "darrell henderson jr": ["darrell henderson jr"],
  "darren waller": ["darren waller"],
  "darrynton evans": ["darrynton evans"],
  "davante adams": ["davante adams"],
  "david bell": ["david bell"],
  "david montgomery": ["david montgomery"],
  "david moore": ["david moore"],
  "david njoku": ["david njoku"],
  "davis allen": ["davis allen"],
  "davis mills": ["davis mills"],
  "davis price": ["tyrion davis price"],
  "dawson knox": ["dawson knox"],
  "de von achane": ["de'von achane"],
  "de'von achane": ["de'von achane"],
  "deandre carter": ["deandre carter"],
  "deandre hopkins": ["deandre hopkins"],
  "dee eskridge": ["dee eskridge"],
  "deebo samuel": ["deebo samuel"],
  "deejay dallas": ["deejay dallas"],
  deguara: ["josiah deguara"],
  dell: ["tank dell"],
  "demarcus robinson": ["demarcus robinson"],
  "demario douglas": ["demario douglas"],
  demercado: ["emari demercado"],
  "deon jackson": ["deon jackson"],
  "deonte harty": ["deonte harty"],
  "derek carr": ["derek carr"],
  "derius davis": ["derius davis"],
  "derrick gore": ["derrick gore"],
  "derrick henry": ["derrick henry"],
  "deshaun watson": ["deshaun watson"],
  "desmond ridder": ["desmond ridder"],
  "deuce vaughn": ["deuce vaughn"],
  "devante parker": ["devante parker"],
  "deven thompkins": ["deven thompkins"],
  "devin duvernay": ["devin duvernay"],
  "devin singletary": ["devin singletary"],
  "devine ozigbo": ["devine ozigbo"],
  devito: ["tommy devito"],
  "devonta smith": ["devonta smith"],
  diggs: ["stefon diggs"],
  dillon: ["aj dillon"],
  "diontae johnson": ["diontae johnson"],
  dissly: ["will dissly"],
  "dj chark": ["dj chark"],
  "dj moore": ["dj moore"],
  "dk metcalf": ["dk metcalf"],
  dobbins: ["j k dobbins"],
  dobbs: ["joshua dobbs"],
  "donald parham jr": ["donald parham jr"],
  "donovan peoples jones": ["donovan peoples jones"],
  "dontayvion wicks": ["dontayvion wicks"],
  "dorian thompson robinson": ["dorian thompson robinson"],
  dortch: ["greg dortch"],
  doubs: ["romeo doubs"],
  douglas: ["demario douglas"],
  dowdle: ["rico dowdle"],
  downs: ["josh downs"],
  drake: ["kenyan drake"],
  "drake london": ["drake london"],
  "drew lock": ["drew lock"],
  "drew ogletree": ["drew ogletree"],
  "drew sample": ["drew sample"],
  driskel: ["jeff driskel"],
  durham: ["payne durham"],
  "durham smythe": ["durham smythe"],
  duvernay: ["devin duvernay"],
  "dyami brown": ["dyami brown"],
  "easton stick": ["easton stick"],
  edmonds: ["chase edmonds"],
  edwards: ["gus edwards"],
  "edwards helaire": ["clyde edwards helaire"],
  ekeler: ["austin ekeler"],
  "elijah dotson": ["elijah dotson"],
  "elijah higgins": ["elijah higgins"],
  "elijah mitchell": ["elijah mitchell"],
  "elijah moore": ["elijah moore"],
  elliott: ["ezekiel elliott"],
  "emanuel wilson": ["emanuel wilson"],
  "emari demercado": ["emari demercado"],
  engram: ["evan engram"],
  "equanimeous st brown": ["equanimeous st brown"],
  "eric gray": ["eric gray"],
  erickson: ["alex erickson"],
  "erik ezukanma": ["erik ezukanma"],
  ertz: ["zach ertz"],
  eskridge: ["dee eskridge"],
  "etienne jr": ["travis etienne jr"],
  "evan engram": ["evan engram"],
  "evan hull": ["evan hull"],
  everett: ["gerald everett"],
  "ezekiel elliott": ["ezekiel elliott"],
  "ezekiel turner": ["ezekiel turner"],
  ezukanma: ["erik ezukanma"],
  fant: ["noah fant"],
  farrell: ["luke farrell"],
  ferguson: ["jake ferguson"],
  fields: ["justin fields"],
  flacco: ["joe flacco"],
  flowers: ["zay flowers"],
  ford: ["jerome ford"],
  foreman: ["d'onta foreman"],
  "foster moreau": ["foster moreau"],
  fournette: ["leonard fournette"],
  freeman: ["royce freeman"],
  freiermuth: ["pat freiermuth"],
  funk: ["jake funk"],
  gabbert: ["blaine gabbert"],
  "gabe davis": ["gabe davis"],
  gainwell: ["kenneth gainwell"],
  gallup: ["michael gallup"],
  "gardner minshew": ["gardner minshew"],
  garoppolo: ["jimmy garoppolo"],
  "garrett wilson": ["garrett wilson"],
  "gary brightwell": ["gary brightwell"],
  "geno smith": ["geno smith"],
  "geoff swaim": ["geoff swaim"],
  "george kittle": ["george kittle"],
  "george pickens": ["george pickens"],
  "gerald everett": ["gerald everett"],
  gesicki: ["mike gesicki"],
  gibbs: ["jahmyr gibbs"],
  gibson: ["antonio gibson"],
  gillikin: ["blake gillikin"],
  gipson: ["xavier gipson"],
  "godwin jr": ["chris godwin jr"],
  goedert: ["dallas goedert"],
  goff: ["jared goff"],
  goodson: ["tyler goodson"],
  goodwin: ["marquise goodwin"],
  "gordon iii": ["melvin gordon iii"],
  gore: ["derrick gore"],
  granson: ["kylen granson"],
  "greg dortch": ["greg dortch"],
  "gus edwards": ["gus edwards"],
  guyton: ["jalen guyton"],
  ham: ["c j ham"],
  "hardman jr": ["mecole hardman jr"],
  "harrison bryant": ["harrison bryant"],
  harty: ["deonte harty"],
  "hayden hurst": ["hayden hurst"],
  heath: ["malik heath"],
  heinicke: ["taylor heinicke"],
  hekker: ["johnny hekker"],
  hendershot: ["peyton hendershot"],
  "henderson jr": ["darrell henderson jr"],
  heyward: ["connor heyward"],
  higbee: ["tyler higbee"],
  hockenson: ["t j hockenson"],
  hodge: ["khadarel hodge"],
  hodgins: ["isaiah hodgins"],
  hollins: ["mack hollins"],
  "hollywood brown": ["hollywood brown"],
  hooker: ["amani hooker"],
  hooper: ["austin hooper"],
  howell: ["sam howell"],
  hoyer: ["brian hoyer"],
  hubbard: ["chuba hubbard"],
  hudson: ["tanner hudson"],
  hull: ["evan hull"],
  humphrey: ["lil'jordan humphrey"],
  hunt: ["kareem hunt"],
  "hunter henry": ["hunter henry"],
  "hunter luepke": ["hunter luepke"],
  "hunter renfrow": ["hunter renfrow"],
  huntley: ["tyler huntley"],
  hurst: ["hayden hurst"],
  hurts: ["jalen hurts"],
  hutchinson: ["xavier hutchinson"],
  hyatt: ["jalin hyatt"],
  "ian thomas": ["ian thomas"],
  "ihmir smith marsette": ["ihmir smith marsette"],
  ingold: ["alec ingold"],
  ingram: ["keaontay ingram"],
  iosivas: ["andrei iosivas"],
  "irv smith jr": ["irv smith jr"],
  irwin: ["trenton irwin"],
  "isaiah hodgins": ["isaiah hodgins"],
  "isaiah likely": ["isaiah likely"],
  "isaiah mckenzie": ["isaiah mckenzie"],
  "isaiah spiller": ["isaiah spiller"],
  "isiah pacheco": ["isiah pacheco"],
  "israel abanikanda": ["israel abanikanda"],
  "j k dobbins": ["j k dobbins"],
  "ja marr chase": ["ja'marr chase"],
  "ja'marr chase": ["ja'marr chase"],
  jacobs: ["josh jacobs"],
  "jacoby brissett": ["jacoby brissett"],
  "jahan dotson": ["jahan dotson"],
  "jahmyr gibbs": ["jahmyr gibbs"],
  "jake bobo": ["jake bobo"],
  "jake browning": ["jake browning"],
  "jake ferguson": ["jake ferguson"],
  "jake funk": ["jake funk"],
  "jakobi meyers": ["jakobi meyers"],
  "jaleel mclaughlin": ["jaleel mclaughlin"],
  "jalen brooks": ["jalen brooks"],
  "jalen guyton": ["jalen guyton"],
  "jalen hurts": ["jalen hurts"],
  "jalen reagor": ["jalen reagor"],
  "jalen reeves maybin": ["jalen reeves maybin"],
  "jalen tolbert": ["jalen tolbert"],
  "jalin hyatt": ["jalin hyatt"],
  "jamaal williams": ["jamaal williams"],
  "jamal agnew": ["jamal agnew"],
  "jameis winston": ["jameis winston"],
  james: ["richie james"],
  "james conner": ["james conner"],
  "james cook iii": ["james cook iii"],
  "james robinson": ["james robinson"],
  "jameson williams": ["jameson williams"],
  "jamison crowder": ["jamison crowder"],
  "jared goff": ["jared goff"],
  "jaren hall": ["jaren hall"],
  jarrett: ["rakim jarrett"],
  "jarrett stidham": ["jarrett stidham"],
  "jashaun corbin": ["jashaun corbin"],
  "jason brownlee": ["jason brownlee"],
  "jason cabinda": ["jason cabinda"],
  "jauan jennings": ["jauan jennings"],
  "javonte williams": ["javonte williams"],
  "jaxon smith njigba": ["jaxon smith njigba"],
  "jayden reed": ["jayden reed"],
  "jaylen waddle": ["jaylen waddle"],
  "jaylen warren": ["jaylen warren"],
  "jeff driskel": ["jeff driskel"],
  "jeff wilson jr": ["jeff wilson jr"],
  jennings: ["jauan jennings"],
  "jeremy ruckert": ["jeremy ruckert"],
  "jerick mckinnon": ["jerick mckinnon"],
  "jerome ford": ["jerome ford"],
  "jerry jeudy": ["jerry jeudy"],
  jeudy: ["jerry jeudy"],
  "jimmy garoppolo": ["jimmy garoppolo"],
  "joe burrow": ["joe burrow"],
  "joe flacco": ["joe flacco"],
  "joe mixon": ["joe mixon"],
  "john bates": ["john bates"],
  "john metchie iii": ["john metchie iii"],
  "johnny hekker": ["johnny hekker"],
  "johnny mundt": ["johnny mundt"],
  "jonathan mingo": ["jonathan mingo"],
  "jonathan taylor": ["jonathan taylor"],
  "jonathan ward": ["jonathan ward"],
  "jonathan williams": ["jonathan williams"],
  "jones sr": ["aaron jones sr"],
  "jonnu smith": ["jonnu smith"],
  jordan: ["brevin jordan"],
  "jordan addison": ["jordan addison"],
  "jordan akins": ["jordan akins"],
  "jordan love": ["jordan love"],
  "jordan mason": ["jordan mason"],
  "josh allen": ["josh allen"],
  "josh downs": ["josh downs"],
  "josh jacobs": ["josh jacobs"],
  "josh oliver": ["josh oliver"],
  "josh reynolds": ["josh reynolds"],
  "josh whyle": ["josh whyle"],
  "joshua dobbs": ["joshua dobbs"],
  "joshua kelley": ["joshua kelley"],
  "joshua palmer": ["joshua palmer"],
  "josiah deguara": ["josiah deguara"],
  "juju smith schuster": ["juju smith schuster"],
  "julio jones": ["julio jones"],
  "justice hill": ["justice hill"],
  "justin fields": ["justin fields"],
  "justin herbert": ["justin herbert"],
  "justin jefferson": ["justin jefferson"],
  "justin watson": ["justin watson"],
  "justyn ross": ["justyn ross"],
  juszczyk: ["kyle juszczyk"],
  "juwan johnson": ["juwan johnson"],
  "k j osborn": ["k j osborn"],
  "kadarius toney": ["kadarius toney"],
  "kalif raymond": ["kalif raymond"],
  kamara: ["alvin kamara"],
  "kareem hunt": ["kareem hunt"],
  "kavontae turpin": ["kavontae turpin"],
  "ke shawn vaughn": ["ke'shawn vaughn"],
  "ke'shawn vaughn": ["ke'shawn vaughn"],
  "keaontay ingram": ["keaontay ingram"],
  "keaton mitchell": ["keaton mitchell"],
  "keenan allen": ["keenan allen"],
  keenum: ["case keenum"],
  "keisean nixon": ["keisean nixon"],
  kelce: ["travis kelce"],
  kelley: ["joshua kelley"],
  "kendre miller": ["kendre miller"],
  "kendrick bourne": ["kendrick bourne"],
  "kene nwangwu": ["kene nwangwu"],
  "kenneth gainwell": ["kenneth gainwell"],
  "kenneth walker iii": ["kenneth walker iii"],
  "kenny pickett": ["kenny pickett"],
  "kenyan drake": ["kenyan drake"],
  "kevin harris": ["kevin harris"],
  "khadarel hodge": ["khadarel hodge"],
  "khalil herbert": ["khalil herbert"],
  "khalil shakir": ["khalil shakir"],
  "khari blasingame": ["khari blasingame"],
  kincaid: ["dalton kincaid"],
  kirk: ["christian kirk"],
  "kirk cousins": ["kirk cousins"],
  kittle: ["george kittle"],
  kmet: ["cole kmet"],
  knight: ["bam knight"],
  knox: ["dawson knox"],
  kolar: ["charlie kolar"],
  kraft: ["tucker kraft"],
  krull: ["lucas krull"],
  kupp: ["cooper kupp"],
  "kyle allen": ["kyle allen"],
  "kyle juszczyk": ["kyle juszczyk"],
  "kyle philips": ["kyle philips"],
  "kyle pitts sr": ["kyle pitts sr"],
  "kyle trask": ["kyle trask"],
  "kylen granson": ["kylen granson"],
  "kyler murray": ["kyler murray"],
  "kyren williams": ["kyren williams"],
  "la mical perine": ["la'mical perine"],
  "la'mical perine": ["la'mical perine"],
  "lamar jackson": ["lamar jackson"],
  lamb: ["ceedee lamb"],
  laporta: ["sam laporta"],
  "latavius murray": ["latavius murray"],
  "laviska shenault jr": ["laviska shenault jr"],
  lawrence: ["trevor lawrence"],
  lazard: ["allen lazard"],
  "leonard fournette": ["leonard fournette"],
  levis: ["will levis"],
  likely: ["isaiah likely"],
  "lil jordan humphrey": ["lil'jordan humphrey"],
  "lil'jordan humphrey": ["lil'jordan humphrey"],
  lock: ["drew lock"],
  lockett: ["tyler lockett"],
  "logan cooke": ["logan cooke"],
  "logan thomas": ["logan thomas"],
  "logan woodside": ["logan woodside"],
  london: ["drake london"],
  love: ["jordan love"],
  "lucas krull": ["lucas krull"],
  luepke: ["hunter luepke"],
  "luke farrell": ["luke farrell"],
  "luke musgrave": ["luke musgrave"],
  "luke schoonmaker": ["luke schoonmaker"],
  "lynn bowden jr": ["lynn bowden jr"],
  "mac jones": ["mac jones"],
  "mack hollins": ["mack hollins"],
  mahomes: ["patrick mahomes"],
  "malik cunningham": ["malik cunningham"],
  "malik heath": ["malik heath"],
  "malik willis": ["malik willis"],
  mallory: ["will mallory"],
  mann: ["braden mann"],
  "marcus mariota": ["marcus mariota"],
  mariota: ["marcus mariota"],
  "mark andrews": ["mark andrews"],
  "marquez valdes scantling": ["marquez valdes scantling"],
  "marquise goodwin": ["marquise goodwin"],
  "marshall jr": ["terrace marshall jr"],
  "marvin mims jr": ["marvin mims jr"],
  mason: ["jordan mason"],
  "mason rudolph": ["mason rudolph"],
  "matt barkley": ["matt barkley"],
  "matt breida": ["matt breida"],
  "matthew stafford": ["matthew stafford"],
  mattison: ["alexander mattison"],
  mayer: ["michael mayer"],
  mayfield: ["baker mayfield"],
  mcbride: ["trey mcbride"],
  mccaffrey: ["christian mccaffrey"],
  mccarron: ["aj mccarron"],
  "mccloud iii": ["ray ray mccloud iii"],
  mckenzie: ["isaiah mckenzie"],
  mckinnon: ["jerick mckinnon"],
  mclaughlin: ["jaleel mclaughlin"],
  mclaurin: ["terry mclaurin"],
  "mecole hardman jr": ["mecole hardman jr"],
  melton: ["bo melton"],
  "melvin gordon iii": ["melvin gordon iii"],
  metcalf: ["dk metcalf"],
  "metchie iii": ["john metchie iii"],
  meyers: ["jakobi meyers"],
  "michael burton": ["michael burton"],
  "michael carter": ["michael carter"],
  "michael gallup": ["michael gallup"],
  "michael mayer": ["michael mayer"],
  "michael pittman jr": ["michael pittman jr"],
  "michael thomas": ["michael thomas"],
  "michael wilson": ["michael wilson"],
  "mike boone": ["mike boone"],
  "mike evans": ["mike evans"],
  "mike gesicki": ["mike gesicki"],
  "mike white": ["mike white"],
  "mike williams": ["mike williams"],
  "miles sanders": ["miles sanders"],
  mills: ["davis mills"],
  "mims jr": ["marvin mims jr"],
  mingo: ["jonathan mingo"],
  minshew: ["gardner minshew"],
  "mitchell trubisky": ["mitchell trubisky"],
  "mitchell wilcox": ["mitchell wilcox"],
  mixon: ["joe mixon"],
  "mo alie cox": ["mo alie cox"],
  "montgomery ii": ["ty montgomery ii"],
  mooney: ["darnell mooney"],
  moreau: ["foster moreau"],
  morstead: ["thomas morstead"],
  moss: ["zack moss"],
  mostert: ["raheem mostert"],
  mullens: ["nick mullens"],
  mundt: ["johnny mundt"],
  musgrave: ["luke musgrave"],
  "mycole pruitt": ["mycole pruitt"],
  nacua: ["puka nacua"],
  "najee harris": ["najee harris"],
  "nathan peterman": ["nathan peterman"],
  "nelson agholor": ["nelson agholor"],
  "nick bawden": ["nick bawden"],
  "nick chubb": ["nick chubb"],
  "nick mullens": ["nick mullens"],
  "nick niemann": ["nick niemann"],
  "nick westbrook ikhine": ["nick westbrook ikhine"],
  "nico collins": ["nico collins"],
  niemann: ["nick niemann"],
  nixon: ["keisean nixon"],
  njoku: ["david njoku"],
  "noah brown": ["noah brown"],
  "noah fant": ["noah fant"],
  "noah gray": ["noah gray"],
  nwangwu: ["kene nwangwu"],
  "o'connell": ["aidan o'connell"],
  "odell beckham jr": ["odell beckham jr"],
  ogletree: ["drew ogletree"],
  ogunbowale: ["dare ogunbowale"],
  okonkwo: ["chig okonkwo"],
  "olamide zaccheaus": ["olamide zaccheaus"],
  olave: ["chris olave"],
  oliver: ["josh oliver"],
  osborn: ["k j osborn"],
  otton: ["cade otton"],
  ozigbo: ["devine ozigbo"],
  pacheco: ["isiah pacheco"],
  "parham jr": ["donald parham jr"],
  parker: ["devante parker"],
  "parker washington": ["parker washington"],
  parkinson: ["colby parkinson"],
  "parris campbell": ["parris campbell"],
  "pat freiermuth": ["pat freiermuth"],
  "patrick mahomes": ["patrick mahomes"],
  "patrick taylor jr": ["patrick taylor jr"],
  patterson: ["cordarrelle patterson"],
  "payne durham": ["payne durham"],
  penny: ["rashaad penny"],
  "peoples jones": ["donovan peoples jones"],
  perry: ["a t perry"],
  peterman: ["nathan peterman"],
  "peyton hendershot": ["peyton hendershot"],
  "pharaoh brown": ["pharaoh brown"],
  philips: ["kyle philips"],
  pickens: ["george pickens"],
  pickett: ["kenny pickett"],
  "pierre strong jr": ["pierre strong jr"],
  "pittman jr": ["michael pittman jr"],
  "pitts sr": ["kyle pitts sr"],
  "pj walker": ["pj walker"],
  pollard: ["tony pollard"],
  powell: ["brandon powell"],
  prentice: ["adam prentice"],
  prescott: ["dak prescott"],
  pringle: ["byron pringle"],
  pruitt: ["mycole pruitt"],
  "puka nacua": ["puka nacua"],
  purdy: ["brock purdy"],
  "quentin johnston": ["quentin johnston"],
  "quez watkins": ["quez watkins"],
  "rachaad white": ["rachaad white"],
  "raheem blackshear": ["raheem blackshear"],
  "raheem mostert": ["raheem mostert"],
  "rakim jarrett": ["rakim jarrett"],
  "rashaad penny": ["rashaad penny"],
  "rashee rice": ["rashee rice"],
  "rashid shaheed": ["rashid shaheed"],
  "rashod bateman": ["rashod bateman"],
  "ray ray mccloud iii": ["ray ray mccloud iii"],
  raymond: ["kalif raymond"],
  reagor: ["jalen reagor"],
  reed: ["jayden reed"],
  "reeves maybin": ["jalen reeves maybin"],
  renfrow: ["hunter renfrow"],
  "rhamondre stevenson": ["rhamondre stevenson"],
  rice: ["rashee rice"],
  "richardson sr": ["anthony richardson sr"],
  "richie james": ["richie james"],
  "rico dowdle": ["rico dowdle"],
  ridder: ["desmond ridder"],
  ridley: ["calvin ridley"],
  "river cracraft": ["river cracraft"],
  rivers: ["ronnie rivers"],
  "robbie chosen": ["robbie chosen"],
  "robert tonyan": ["robert tonyan"],
  "robert woods": ["robert woods"],
  "robinson jr": ["brian robinson jr"],
  rodgers: ["aaron rodgers"],
  "rodriguez jr": ["chris rodriguez jr"],
  "romeo doubs": ["romeo doubs"],
  "rondale moore": ["rondale moore"],
  "ronnie bell": ["ronnie bell"],
  "ronnie rivers": ["ronnie rivers"],
  "roschon johnson": ["roschon johnson"],
  ross: ["justyn ross"],
  "royce freeman": ["royce freeman"],
  ruckert: ["jeremy ruckert"],
  rudolph: ["mason rudolph"],
  rush: ["cooper rush"],
  "russell wilson": ["russell wilson"],
  "ryan stonehouse": ["ryan stonehouse"],
  "ryan tannehill": ["ryan tannehill"],
  rypien: ["brett rypien"],
  "salvon ahmed": ["salvon ahmed"],
  "sam darnold": ["sam darnold"],
  "sam howell": ["sam howell"],
  "sam laporta": ["sam laporta"],
  "samaje perine": ["samaje perine"],
  "samori toure": ["samori toure"],
  sample: ["drew sample"],
  sanders: ["miles sanders"],
  "saquon barkley": ["saquon barkley"],
  schoonmaker: ["luke schoonmaker"],
  schultz: ["dalton schultz"],
  "scotty miller": ["scotty miller"],
  "sean clifford": ["sean clifford"],
  "sean tucker": ["sean tucker"],
  sermon: ["trey sermon"],
  shaheed: ["rashid shaheed"],
  shakir: ["khalil shakir"],
  "shenault jr": ["laviska shenault jr"],
  shepard: ["sterling shepard"],
  "sherfield sr": ["trent sherfield sr"],
  siemian: ["trevor siemian"],
  sims: ["steven sims"],
  singletary: ["devin singletary"],
  skowronek: ["ben skowronek"],
  "skyy moore": ["skyy moore"],
  slayton: ["darius slayton"],
  smartt: ["stone smartt"],
  "smith jr": ["irv smith jr"],
  "smith marsette": ["ihmir smith marsette"],
  "smith njigba": ["jaxon smith njigba"],
  "smith schuster": ["juju smith schuster"],
  smythe: ["durham smythe"],
  spears: ["tyjae spears"],
  spiller: ["isaiah spiller"],
  stafford: ["matthew stafford"],
  "stefon diggs": ["stefon diggs"],
  "stephen sullivan": ["stephen sullivan"],
  "sterling shepard": ["sterling shepard"],
  "steven sims": ["steven sims"],
  stevenson: ["rhamondre stevenson"],
  stick: ["easton stick"],
  stidham: ["jarrett stidham"],
  "stone smartt": ["stone smartt"],
  stonehouse: ["ryan stonehouse"],
  "strong jr": ["pierre strong jr"],
  stroud: ["c j stroud"],
  sullivan: ["stephen sullivan"],
  sutton: ["courtland sutton"],
  swaim: ["geoff swaim"],
  swift: ["d'andre swift"],
  "t j hockenson": ["t j hockenson"],
  tagovailoa: ["tua tagovailoa"],
  "tank bigsby": ["tank bigsby"],
  "tank dell": ["tank dell"],
  tannehill: ["ryan tannehill"],
  "tanner hudson": ["tanner hudson"],
  "taylor heinicke": ["taylor heinicke"],
  "taylor jr": ["patrick taylor jr"],
  "taysom hill": ["taysom hill"],
  "teddy bridgewater": ["teddy bridgewater"],
  "tee higgins": ["tee higgins"],
  "terrace marshall jr": ["terrace marshall jr"],
  "terry mclaurin": ["terry mclaurin"],
  thielen: ["adam thielen"],
  "thomas morstead": ["thomas morstead"],
  thompkins: ["deven thompkins"],
  "thompson robinson": ["dorian thompson robinson"],
  thornton: ["tyquan thornton"],
  tillman: ["cedric tillman"],
  "tim boyle": ["tim boyle"],
  "tim jones": ["tim jones"],
  tolbert: ["jalen tolbert"],
  "tommy devito": ["tommy devito"],
  "tommy townsend": ["tommy townsend"],
  "tommy tremble": ["tommy tremble"],
  toney: ["kadarius toney"],
  "tony jones jr": ["tony jones jr"],
  "tony pollard": ["tony pollard"],
  tonyan: ["robert tonyan"],
  toure: ["samori toure"],
  townsend: ["tommy townsend"],
  trask: ["kyle trask"],
  trautman: ["adam trautman"],
  "travis etienne jr": ["travis etienne jr"],
  "travis kelce": ["travis kelce"],
  "trayveon williams": ["trayveon williams"],
  "tre tucker": ["tre tucker"],
  tremble: ["tommy tremble"],
  "trent sherfield sr": ["trent sherfield sr"],
  "trent taylor": ["trent taylor"],
  "trenton irwin": ["trenton irwin"],
  "tress way": ["tress way"],
  "trevor lawrence": ["trevor lawrence"],
  "trevor siemian": ["trevor siemian"],
  "trey mcbride": ["trey mcbride"],
  "trey palmer": ["trey palmer"],
  "trey sermon": ["trey sermon"],
  "treylon burks": ["treylon burks"],
  trubisky: ["mitchell trubisky"],
  "tua tagovailoa": ["tua tagovailoa"],
  "tucker kraft": ["tucker kraft"],
  tune: ["clayton tune"],
  turpin: ["kavontae turpin"],
  "tutu atwell": ["tutu atwell"],
  "ty chandler": ["ty chandler"],
  "ty johnson": ["ty johnson"],
  "ty montgomery ii": ["ty montgomery ii"],
  "ty zentner": ["ty zentner"],
  "tyjae spears": ["tyjae spears"],
  "tyler allgeier": ["tyler allgeier"],
  "tyler boyd": ["tyler boyd"],
  "tyler conklin": ["tyler conklin"],
  "tyler goodson": ["tyler goodson"],
  "tyler higbee": ["tyler higbee"],
  "tyler huntley": ["tyler huntley"],
  "tyler lockett": ["tyler lockett"],
  "tyler scott": ["tyler scott"],
  "tyquan thornton": ["tyquan thornton"],
  "tyreek hill": ["tyreek hill"],
  "tyrion davis price": ["tyrion davis price"],
  "tyrod taylor": ["tyrod taylor"],
  "tyson bagent": ["tyson bagent"],
  uzomah: ["c j uzomah"],
  "valdes scantling": ["marquez valdes scantling"],
  "van jefferson": ["van jefferson"],
  "velus jones jr": ["velus jones jr"],
  waddle: ["jaylen waddle"],
  walker: ["pj walker"],
  "walker iii": ["kenneth walker iii"],
  waller: ["darren waller"],
  "wan dale robinson": ["wan'dale robinson"],
  "wan'dale robinson": ["wan'dale robinson"],
  ward: ["jonathan ward"],
  warren: ["jaylen warren"],
  watkins: ["quez watkins"],
  way: ["tress way"],
  wentz: ["carson wentz"],
  "westbrook ikhine": ["nick westbrook ikhine"],
  whyle: ["josh whyle"],
  wicks: ["dontayvion wicks"],
  wilcox: ["mitchell wilcox"],
  "will dissly": ["will dissly"],
  "will levis": ["will levis"],
  "will mallory": ["will mallory"],
  willis: ["malik willis"],
  winston: ["jameis winston"],
  woods: ["robert woods"],
  woodside: ["logan woodside"],
  wright: ["brock wright"],
  "xavier gipson": ["xavier gipson"],
  "xavier hutchinson": ["xavier hutchinson"],
  young: ["bryce young"],
  zaccheaus: ["olamide zaccheaus"],
  "zach charbonnet": ["zach charbonnet"],
  "zach ertz": ["zach ertz"],
  "zach evans": ["zach evans"],
  "zach wilson": ["zach wilson"],
  "zack moss": ["zack moss"],
  "zamir white": ["zamir white"],
  zappe: ["bailey zappe"],
  "zay flowers": ["zay flowers"],
  "zay jones": ["zay jones"],
  zentner: ["ty zentner"],
};
